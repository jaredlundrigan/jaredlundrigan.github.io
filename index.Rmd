...
I will be using this tool to analyze how popular certain Google searches for different candidates are.
...
Instead, I will use a different method, which involves analyzing the order a candidate's name appears in Google searches that have the names of both candidates. For example, someone searching "Walker Warnock polls", or "Warnock Walker election" might indicate who they may vote for. Seth Stephen-Davidowitz, American data scientist and author, and who created this keyword search strategy, explains in an [article](https://newsroom.ucla.edu/stories/ucla-faculty-voice-can-google-searches-predict-elections) for UCLA newsroom: "Our research suggests that a person is significantly more likely to put the candidate they support first in a search that includes both candidates’ names." So the order in which the candidates were searched could indicate who will win the state.
...
I pulled the search data from searches for the terms "Walker Warnock polls" and "Warnock Walker polls" from within the state of Georgia during the 2 months before the election runoff on December 7th, 2022. I also pulled keyword search data from terms such as:
- "Walker Warnock election"
- "Warnock Walker debate"
- "Walker vs Warnock"
- "Warnock vs Walker polls"
- "Herschel Walker vs Raphael Warnock"
- "Raphael Warnock vs Herschel Walker polls

The idea is that, the more search terms you are able to analyze, the more accurate the outcome of the analysis will be. 
...
My analysis rightly predicted Johnson would be win the race, although not by much.
...
The results showed that Beasley slightly out-performed but the results were largely as predicted by polling.
...
This was a strong out-performance by Rubio.
...
This was another strong out-performance, in which Paul ended up getting +7.6% more than expected from the polling numbers, and my analysis also had Paul winning.
...
- As a tool to be used with polling data to predict election outcomes
- As a tool to be used to determine who might outperform their polling numbers
- To analyze elections that have a lot of Google searches
...
- To analyze elections that have little to no search data due to lack of interest or
that take place in smaller populated areas

I believe that more data is needed to conclude with certainty that this tool can be used to 
accurately predict elections, but this analysis is a big step forward.

However, there are some limitations in using Google search data. There are some congressional races
in smaller populated areas that have less search data. Having less data could skew results and 
provide inaccurate predictions. There are also some races that just don't garner as much interest
and would also therefore have less search data to analyze. 

But if there are enough Google searches for an election, I believe this strategy is an excellent choice
in being able to predict elections while being used alongside polling data.
